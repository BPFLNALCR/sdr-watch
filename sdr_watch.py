...
# Optional: native RTL-SDR backend
...
# Native RTL-SDR (direct librtlsdr), bypasses Soapy
python3 sdrwatch.py --start 88e6 --stop 108e6 --step 2.048e6 --samp-rate 2.048e6 \
  --fft 4096 --avg 8 --driver rtlsdr_native --gain 35 --bandplan bandplan.csv
...
# Native RTL-SDR (pyrtlsdr). Optional, enabled with --driver rtlsdr_native
try:
...
class SDRSource:
    ...
    def tune(self, freq_hz: float):
        ...
        # brief settle (increase to help tuner lock more reliably)
        time.sleep(0.10)
...
# ------------------------------
# Native RTL-SDR source (pyrtlsdr)
# ------------------------------

class RTLSDRSource:
    def __init__(self, samp_rate: float, gain: str):
        if not HAVE_RTLSDR:
            raise RuntimeError("pyrtlsdr not installed. Install with: pip3 install pyrtlsdr")
        self.dev = RtlSdr()
        # Conservative defaults to help PLL lock on some R82xx parts
        self.dev.sample_rate = float(samp_rate)
        if isinstance(gain, str) and gain.lower() == "auto":
            # pyrtlsdr supports 'auto' as a string
            self.dev.gain = "auto"
        else:
            self.dev.gain = float(gain)
        # Leave freq correction (ppm) at default; user can set via --ppm later if added

    def tune(self, freq_hz: float):
        self.dev.center_freq = float(freq_hz)
        time.sleep(0.10)

    def read(self, nsamps: int) -> np.ndarray:
        # pyrtlsdr returns complex64 numpy array already
        data = self.dev.read_samples(nsamps)
...
def run(args):
    ...
    try:
        ...
        while center <= args.stop:
            ...
            # Step across the band
            center += args.step
            # Note: we intentionally do NOT break after one window.
            # A single "sweep" means traversing the entire [start, stop] range once.
...
def parse_args():
    ...
    p.add_argument("--driver", type=str, default="rtlsdr", help="Soapy driver key (rtlsdr, hackrf, airspy, etc.) or 'rtlsdr_native' for direct librtlsdr")
